      ...
      <div class="mx-[5%] md:mx-0">
        ...
        <section class="mb-16">
          <div className='flex justify-between'>
            <h2 class="mb-5 text-xl font-bold">Latest Articles</h2>
            <button onClick={ toggleViewMore } class="mb-1 text-primary dark:text-primary-400 text-sm underline"><strong>View {viewMore ? 'Less' : 'More'}</strong></button>
          </div>

          <div class="grid gap-6 xl:gap-x-12 lg:grid-cols-3 lg:grid-flow-row md:gap-8">
              ...
              <div class={index === 0 ? "row-span-2" : ""}>
                ...
                <div className="flex justify-between">
                  <div>
                    ...
                    <div className="flex">
                      <div class="mb-3 flex items-center justify-center text-sm font-medium text-primary md:justify-start">
                      ...
                      {item.sponsored ? <div class="mx-2 mb-3 flex items-center justify-center text-sm font-medium text-yellow-600 md:justify-start">
                  ...
                  <div class="hidden lg:block mb-6 w-[35%]">
                  {index !== 0 ? <div class="relative overflow-hidden rounded-lg bg-cover bg-no-repeat shadow-lg dark:shadow-black/20" data-te-ripple-init data-te-ripple-color="light">
                  <img src={item.image} class="w-full" alt={item.title} />
                  <a href="#!">
                    <div class="absolute top-0 right-0 bottom-0 left-0 h-full w-full overflow-hidden bg-fixed opacity-0 transition duration-300 ease-in-out hover:opacity-100 bg-[hsla(0,0%,98.4%,.15)]"></div>
                  </a>
                ...
                <p class="mb-6 mt-2 text-neutral-500 dark:text-neutral-300">
        ...
        <section class="mb-32">
          <div className='flex justify-between'>
            <h2 class="mb-5 text-xl font-bold">Sponsored Content</h2>
            <button onClick={toggleViewMore2} class="mb-1 text-primary dark:text-primary-400 text-sm underline"><strong>View {viewMore2 ? 'Less' : 'More'}</strong></button>
          </div>
          ...
          {data.filter(item => item.sponsored).slice(0, viewMore2 ? data.length : 3).map((item, index) => (
            <div class="mb-6 lg:mb-0" key={index}>
              ...
              <h5 class="mb-3 text-lg font-bold">{item.title}</h5>
              <div class="mb-3 flex text-sm font-medium text-yellow-600">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="mr-2 h-5 w-5">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 18.75a60.07 60.07 0 0115.797 2.101c.727.198 1.453-.342 1.453-1.096V18.75M3.75 4.5v.75A.75.75 0 013 6h-.75m0 0v-.375c0-.621.504-1.125 1.125-1.125H20.25M2.25 6v9m18-10.5v.75c0 .414.336.75.75.75h.75m-1.5-1.5h.375c.621 0 1.125.504 1.125 1.125v9.75c0 .621-.504 1.125-1.125 1.125h-.375m1.5-1.5H21a.75.75 0 00-.75.75v.75m0 0H3.75m0 0h-.375a1.125 1.125 0 01-1.125-1.125V15m1.5 1.5v-.75A.75.75 0 003 15h-.75M15 10.5a3 3 0 11-6 0 3 3 0 016 0zm3 0h.008v.008H18V10.5zm-12 0h.008v.008H6V10.5z" />
                </svg>
                {item.category}
